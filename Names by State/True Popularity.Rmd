...
state_df <- read.csv(paste(params$state,'.csv',sep=''), header=FALSE)
...
add_rank_change_col <- function(df) {
  df %>%
    group_by(name,sex) %>%
...
create_stacked_area <- function(df,sex_code) {
  add_labs(add_geoms_area_by_rank(create_plot_popularity_index(df,sex_code)),sex_code) +
    labs(y=paste('percent of ',sexes[sex_code],'s with a rank <= ',params$rank,' name',sep=''))
}
